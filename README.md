...
The **Stream of Frequency Mix Archive Manager** provides an interactive, terminal-driven control center (**78 operations** across **7 logical relational sections**) that automates the entire lifecycle of professional DJ mixes and audio recordings:
...
## 🎛️ Feature Matrix (78 Core Operations in 7 Logical Sections)
...
| **49**| **Record Video of DJ Mix using GPU Screen Recorder (Linux)** | Main menu Section 2, option 21. Opens GPU Screen Recorder in a new desktop window on Linux (`gpu-screen-recorder-gtk`, or Flatpak `com.dec05eba.gpu_screen_recorder`) and returns to the menu. Direct shortcut: `gpu-screen-recorder`. |

### ─── [ SECTION 5: LIVE MONITORS & SYSTEM DIAGNOSTICS ] ────────
| # | Operation | Description |
|---|---|---|
| **50**| **Launch Live Tracklist Monitor** | Real-time CLI display connecting to Strawberry MPRIS and `cliamp` with live progress and track info (`SOF_Live_Tracker.sh`). |
...
| **52**| **Launch Live File Transfer Monitor** | Inspects transfer speeds, byte positions, and percentage for huge files (`transfer-monitor`). |
| **53**| **Launch Chrome Upload Monitor** | Monitors web uploads (e.g. Apple Podcasts Connect, YouTube Studio) in real-time (`chrome_upload_monitor.py`). |
| **54**| **View Advanced Archive Statistics** | Deep inventory scan calculating total duration, file sizes, GB footprint, and tracklist completeness (`SOF_Archive_Stats.sh`). |
| **55**| **View Running Background Tasks** | Scans process table for active encoding, syncing, or AI batch jobs. |
| **56**| **Launch Resource Monitor** | Launches `btop` for deep multi-core CPU and memory profiling. |
| **57**| **Launch GPU Process Monitor** | Launches `nvtop` for real-time monitoring of NVIDIA GPU clock, VRAM, and power draw. |
| **58**| **Launch System Process Monitor** | Quick-launches `top` inside manager session. |

### ─── [ SECTION 6: SYSTEM, NETWORK & HARDWARE MANAGEMENT ] ─────
| # | Operation | Description |
|---|---|---|
| **59**| **Manage WAN2GP Server** | Controls WAN2GP AI video server (Profile 2 / 4.5, Flux Klein 9B batch, LTX Video 2B/13B). Accessible via KDE Connect shortcuts, desktop entries, and CLI (`--wan2gp-start-4.5` / `--wan2gp-stop` / `58 2` / `58 3`). |
...
| **67**| **Audio / Sound Settings (OS Tailored)** | Opens Plasma X11 on Linux, Audio MIDI Setup on macOS, or Windows Sound Panel (`control.exe mmsys.cpl`). |
| **68**| **Close All Desktop Applications** | Gracefully closes external desktop windows using AppleScript (macOS), PowerShell (Windows), or `wmctrl` (Linux) while shielding the manager. |
| **69**| **System Maintenance & Cleanup** | Executes platform maintenance (Linux `ujust clean-system`, macOS `brew cleanup` & RAM purge, Windows `winget upgrade` & temp cleanup, FreeBSD `pkg clean`, `pkg upgrade`, `pkg autoremove`). |
| **70**| **Launch GeeXLab Demo Launcher** | Runs 3D/OpenGL shader demos and GPU stress tests via GeeXLab/FurMark. |
| **71**| **Burn ISO Image to USB Drive** | Writes bootable ISO files directly to removable USB storage with safety checks and dd progress (macOS `diskutil` / Linux `lsblk`). |
| **72**| **Dynamic System MOTD Banner Manager** | Dynamic Message Of The Day generator (`update_system_motd.sh`): renders stylized ANSI MOTD table summarizing the last 3 created mixes, dates, times, sizes, formats, and full file names. |

### ─── [ SECTION 7: AI, SHELL CLI & SETTINGS ] ───────────────────
| # | Operation | Description |
|---|---|---|
| **73**| **Launch AI Assistant / Models (AGY)** | Starts Antigravity CLI AI sessions (Claude Sonnet, Claude Opus, GPT-OSS, Gemini), manages local Ollama models, or launches DeepSeek Harness (`dsh-mobile`). |
| **74**| **Run Bash CLI Commands** | Built-in interactive Bash shell and direct command execution runner. |
| **75**| **Manager Themes & Color Palette Switcher** | Switch between 9 terminal themes (Cyberpunk, Dracula, Nord, Matrix, Solarized, Tokyo Night, Monokai, Gruvbox, Emerald, Classic). |
...
| **77**| **Reboot System** | Cross-platform system reboot with safety confirmation dialog (`systemctl reboot`, macOS `osascript`, Windows `shutdown.exe /r`, FreeBSD `shutdown -r now`). |
